...
public class GenericMapper extends Mapper<LongWritable, Text, Text, Text> {
    ...
    protected void setup(Context context) throws IOException,
        ...
        Hadoopizer.logger.info("Looking for static input files in work dir: "
                + workDir.getAbsolutePath());
        Pattern p = Pattern.compile("static_data__(.*)__.*"); // FIXME bouh,
                                                              // hard coded
        Matcher m;
        String fileId;
        for (int i = 0; i < workFiles.length; i++) {
            ...
            if (m.matches()) {
                fileId = m.group(1);
                Hadoopizer.logger.info("Found the static input file '" + fileId
                        + "' in the distributed cache: "
                        + workFiles[i].toString());
        ...
        // This input file will be used in the command line launched in the
        // cleanup step
        inputFile = createTempFile(
                new File(System.getProperty("java.io.tmpdir")), "input",
                ".chunk");
        writer = new BufferedWriter(new FileWriter(inputFile));
        Hadoopizer.logger.info("Writing input chunk to '"
    ...
    protected void map(LongWritable key, Text value, Context context)
        ...
        // context.getCounter(GENERIC_COUNTER.INPUT_RECORDS).increment(1); //
        // TODO this is useless -> find better use cases if needed
    ...
    protected void cleanup(Context context) throws IOException,
        ...
        // TODO maybe add a lock (optional?) to make sure only 1 command running
        // at the same time on each node. This is done by eoulsan, but not
        // necessarily useful.
        ...
        File outputFile = createTempFile(
                new File(System.getProperty("java.io.tmpdir")), "output",
                ".tmp"); // FIXME better tmp dir!
        ...
        if (result != 0) {
            throw new RuntimeException("Execution of command failed (returned "
                    + result + ")");
        }
        ...
        String outputContent = FileUtils.readFileToString(outputFile); // TODO
                                                                       // handle
                                                                       // binary
                                                                       // results
         ...
         * Hadoopizer.logger.warning("Cannot delete output file: " +
     ...
     * 
     * @param directory
     *            parent directory of the temporary file to create
     * @param prefix
     ...
     */
    public static File createTempFile(File directory, String prefix,
        ...
        do {
            ...
            final String filename = prefix + UUID.randomUUID().toString()
                    + suffix;
